    ...
    <form method="POST" action={AppConfig.authenticationEndpoint}>
      ...
      {(showGoogleButton || showGitHubButton) && (
        <>
          <Box>
            {showGoogleButton && (
              <Button fullWidth variant="contained" startIcon={<GoogleIcon />} color="secondary" sx={{ my: 1 }}>
                Continue with Google
              </Button>
            )}
            {showGitHubButton && (
              <Button fullWidth variant="contained" startIcon={<GitHubIcon />} color="secondary" sx={{ my: 1 }}>
