...
-- Table to store service provider (app) properties.
CREATE TABLE SP_APP_PROPERTY (
    ID INTEGER PRIMARY KEY AUTOINCREMENT,
    APP_ID VARCHAR(36) NOT NULL REFERENCES SP_APP(APP_ID) ON DELETE CASCADE,
    PROPERTY_NAME VARCHAR(50) NOT NULL,
    PROPERTY_VALUE VARCHAR(500) NOT NULL,
    IS_SECRET CHAR(1) DEFAULT '0',
    UNIQUE (APP_ID, PROPERTY_NAME)
);

-- Table to store certificates for SP apps.
CREATE TABLE SP_APP_CERTIFICATE (
    ID INTEGER PRIMARY KEY AUTOINCREMENT,
    APP_ID VARCHAR(36) NOT NULL REFERENCES SP_APP(APP_ID) ON DELETE CASCADE,
...
CREATE TABLE IDN_OAUTH_CONSUMER_APPS (
    ...
    TOKEN_ENDPOINT_AUTH_METHODS VARCHAR(255) DEFAULT 'client_secret_post,client_secret_basic'
);
